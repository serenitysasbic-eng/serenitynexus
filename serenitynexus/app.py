...
# --- PANTALLA DE TRANSMISIÓN (VERSIÓN FINAL BLINDADA) ---
    ...
    # 3. PANTALLA DE TRANSMISIÓN (VIDEO FRAGMENTADO)
    if st.session_state.get('f_activo'):
        ...
        for j in range(8):
            ...
            with c_cols[j % 4]:
                html_code = f"""
                <div style="position: relative; background: black; border: 1px solid {color_f}; border-radius: 8px; overflow: hidden; height: 120px;">
                    <div style="position: absolute; top: 5px; left: 8px; z-index: 10; background: rgba(0,0,0,0.6); color: {color_f}; font-family: monospace; font-size: 10px; padding: 2px 5px; border-radius: 3px; font-weight: bold;">
                        {label_nodo}
                    </div>
                    <video width="100%" height="100%" autoplay loop muted playsinline style="object-fit: cover; object-position: {pos};">
                        <source src="{url_video}" type="video/mp4">
                    </video>
                    <div style="position: absolute; bottom: 5px; right: 8px; z-index: 10; color: red; font-family: monospace; font-size: 10px; font-weight: bold; animation: blink 1s infinite alternate;">
                        ● LIVE
                    </div>
